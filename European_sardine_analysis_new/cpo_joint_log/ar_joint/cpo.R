...
library(rjags)
library(mnormt)
source('./cpo_joint_log/ar_joint/draws.R') #saved in draws_posterior
...
log_cpo_i <- function(datamI, datamA, draws, i, log = TRUE) { 
  ...
  for(l in 1 : length(draws[, 1])){
    ...
    for(t in 1 : 34){
      for(s in 1 : 34){
        SigmaI[t, s] <- draws[l, 5]^abs(t - s) * draws[l, 7]^2 *
          sum(draws[l, 5]^(0 : (min(s, t) - 1)))
        
        SigmaA[t, s] <- draws[l, 6]^abs(t - s) * draws[l, 8]^2 *
...
for(s in 1 : dim(data_artisanal)[1]){
  log_cpo[s] <- log_cpo_i(log(data_industrial), log(data_artisanal), 
                          posterior_draws, s)
}

log_cpo_ar_joint <- log_cpo
save(log_cpo_ar_joint, 
     file = './cpo_joint_log/ar_joint/log_cpo_ar_joint.RData')
