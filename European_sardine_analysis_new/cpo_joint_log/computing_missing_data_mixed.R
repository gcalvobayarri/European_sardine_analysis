# Computing missing data from mixed model.
set.seed(6789)
# 1. Loading datata----------------
  ...
  for(t in 1 : (Y - 1)){
    ...
    if(is.na(data_industrial[i, t + 1])){
      data_industrial[i, t +  1] <- exp(rnorm(1,
                                          mean = mean(beta0I) + mean(b0I[i,]) +(mean(beta1I) + mean(b1I[i,])) * t , 
                                          sd = mean(sigmaI)))
    }
    ...
    if(is.na(data_artisanal[i, t + 1])){
      data_artisanal[i, t +  1] <- exp(rnorm(1,
                                         mean = mean(beta0A) + mean(b0A[i,]) + (mean(beta1A) + mean(b1A[i,])) * t, 
                                         sd = mean(sigmaA)))
    }
...
save(data_industrial, data_artisanal, 
     file = './cpo_joint_log/industrial_artisanal_matrix_nonas_mixed.RData')
